...
def generate_demands(nb_demands: int, nb_nodes: int, nb_timesteps: int, max_eprs: int, debug: bool = False) -> list[tuple]:
    ...
    demand_list = list(zip(start_nodes, end_nodes, required_pairs, start_times, end_times)) + list(zip(end_nodes, start_nodes, required_pairs, start_times, end_times))
    if debug: 
        print(f"DEMAND LIST: {demand_list}")
        print(f"DEMAND LIST LENGTH: {len(demand_list)}")
    return demand_list
...
def lp_solve_2(nb_nodes: int, edge_probability: float, min_weight: int, max_weight: int, nb_demands: int, nb_timesteps:int, max_eprs: int, debug: bool = False) -> None:
    ...
    D = [(0, 1, 10, 0, 1), (0, 2, 2, 0, 1), (0, 3, 2, 0, 1), (1, 0, 10, 0, 1), (2, 0, 2, 0, 1), (3, 0, 2, 0, 1)]
    ...
    F = generate_flow_matrix(vars, nb_demands=6, nb_timesteps=2, nb_nodes=7, D=D, debug=debug)
...
def lp_solve(nb_nodes: int, edge_probability: float, min_weight: int, max_weight: int, nb_demands: int, nb_timesteps:int, max_eprs: int, debug: bool = False) -> None:
    ...
    print('----------Solution Details: -----------')
    vars = json.loads(sol_json)['CPLEXSolution']['variables']
    F = generate_flow_matrix(vars, nb_demands=nb_demands, nb_timesteps=nb_timesteps, nb_nodes=nb_nodes, D=D, debug=debug)
    served_count = compute_served(F, D)


if __name__ == "__main__":
    lp_solve(nb_nodes=3, edge_probability=0.3, min_weight=1, max_weight=3, nb_demands=4, nb_timesteps=6, max_eprs=5, debug=True)
