...
def generate_weight_matrix(
    nb_nodes: int, p: float, min_weight: int, max_weight: int, debug: bool = False
) -> np.ndarray:
    ...
    ), f"expected max_weight>=min-weight, got max_weight: {max_weight} and min_weight: {min_weight}"
...
def generate_demands(
    ...
    demand_list = list(
        zip(start_nodes, end_nodes, required_pairs, start_times, end_times)
    )
    return demand_list
...
def define_constraints(
            ...
            for u in range(nb_nodes):
                for v in range(nb_nodes):
                    if W_adj[u][v] != 0:  # edge u->v
                        sum_allocated += variables[(i, t, u, v)]
        model.add_constraint(
            sum_allocated == 0, "no_allocation_bef_after_req_" + str(i)
        )
        # DEBUG PRINTING
        if debug:
    ...
    for i in range(nb_demands):
        ...
        for t in range(demand_list[i][3], demand_list[i][4], 1):
            for u in range(nb_nodes):
                if (
                    W_adj[u][demand_list[i][1]] != 0
                ):  # incoming edge from  u -> dest_i (D[i][1])
                    sum_allocated += variables[(i, t, u, demand_list[i][1])]
        model.add_constraint(
            sum_allocated == source_outflow[i],
            "inflow_dest_demand_" + str(i) + "_dst_" + str(demand_list[i][1]),
        )
        # DEBUG PRINTING
        if debug:
            print(
    ...
    for i in range(nb_demands):
        for t in range(demand_list[i][3], demand_list[i][4], 1):
            ...
            model.add_constraint(
                sum_allocated <= demand_list[i][2],
                "max_allocation_demand_" + str(i) + "_t_" + str(t),
            )
            if debug:
                print(
                    model.get_constraint_by_name(
