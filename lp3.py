...
def define_constraints(model: Model, W_adj: np.ndarray, nb_timesteps: int, nb_demands: int, nb_nodes: int, demand_list: list, variables: dict, nw_state: list, debug: bool = False):
    ...
    #         # DEBUG PRINTING
    #         if debug:
    #             print(f"IFLOW_V_{v}_T_{t} = {inflow_v}")
    #             print(f"OFLOW_V_{v}_T_{t} = {outflow_v}")
        ...
        for u in range(nb_nodes):
         for v in range(nb_nodes):
             if W_adj[u][v] != 0:       #This is an edge
                 ...
                 model.add_constraint(sum_allocated <= nw_state[t][u][v], f"edge_capacity_{u}_{v}_t_{t}")
                 # DEBUG PRINTING
                 if debug:
                    print(model.get_constraint_by_name(f"edge_capacity_{u}_{v}_t_{t}"))
                    print()
...
def generate_flow_matrix(vars: list[dict], nb_demands: int, nb_timesteps: int, nb_nodes: int, debug: bool = False) -> np.ndarray:
...
if __name__ == "__main__":
    lp_solve(nb_nodes=7, edge_probability=0.4, min_weight=1, max_weight=5, nb_demands=3, nb_timesteps=10, max_eprs=4, debug=True)
